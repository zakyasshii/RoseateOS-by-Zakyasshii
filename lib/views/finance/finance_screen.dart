...
class _FinanceScreenState extends State<FinanceScreen> {
  ...
  Future<void> _loadReminders() async {
    ...
    setState(() {
      _reminders = all.where((r) => r.type == 'finance').toList();
    });
  }

  Future<void> _addFinanceReminder(String title, DateTime dateTime, String recurrence, int? customInterval) async {
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          await showDialog(
            context: context,
            builder: (context) => ReminderDialog(
              dialogTitle: 'Add Finance Reminder',
              onSet: _addFinanceReminder,
            ),
          );
        },
